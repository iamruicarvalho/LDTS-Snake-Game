...
> The SnakeGame is basic classic game, where the objective is to be able to capture as much food as you can for the snake and every time the snake eats the size of it grows by one making the game harder and harder as the game progresses. In our Version of the game as the there are 3 levels of difficulty in single player mode. 
This project was developed by Cristiano Rocha (up202108183@fe.up.pt), Rui Carvalho(up202108807@fe.up.pt), Guilherme Ribeiro(up202108731@fe.up.pt) in the context of a pratical work on the subject of LDTS at FEUP(Faculdade de Engenharia da Universidade do Porto).
...
>Keyboard Controls – Functionality of each Button  
 ...
 SnakeSize -> At the top of the screen it is possible to look at the current size of the snake.
...
>Menus -> Menu for GameOver where user can select if he wants to restart or go to main mainMenu. Possibly add option for a multiplayer battle between 2 players. Menu for pause with options to resume and or go back to main mainMenu
  
>Collision Detection – the snake detects that it can’t go out of the Arena she is in and also that it can’t collide with herself  
  
> Food – spawns in not occupied random places on the Arena and respawns when the snake eats it  
  
> Game Information – during gameplay at the top of the screen it will be possible to view the amount of food the snake as eaten and also how much time as passed since start playing  
...
### -The Pattern  
The main pattern we utilize as our architectural pattern was the MVC PATTERN (model-view-control pattern). This pattern is used to separate an application into three main groups of components: Models, Views, and Controllers(creating a more organized structure). Widely used for developing GUI and web applications.  
